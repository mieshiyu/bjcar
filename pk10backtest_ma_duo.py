    ...
    def readdate(self, dateday):
        ...
        with open(dateday) as f:
            ...
            for d in pk10date:
                ...
                if self.huaxian_list_dx.count(1) > 8 or self.huaxian_list_dx.count(1) < 2:
                    self.pianlidu_daxiao += 1
                if self.huaxian_list_s.count(1) > 8 or self.huaxian_list_s.count(1) < 2:
            ...
            print(len(self.zongtongji_list_dx) + len(self.zongtongji_list_s))
            print(
            ...
            self.zhuan_hc.append(self.zhuan)
            for i in self.zhuan_hc:
                self.hejizhuan = self.hejizhuan + i
            print(d['日期'][:5], '单日盈亏', self.zhuan, 'duoloss', self.MA_duo, 'kongloss', self.MA_kong,
                  '总合计盈亏', self.hejizhuan)

            # 多组数据清空列表
            self.pianlidu_danshuang = 0
            self.pianlidu_daxiao = 0
            self.pianlidu = 0
            self.hejizhuan = 0
            self.MA_duo = 0
            self.MA_kong = 0
